              ...
              <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                <form onSubmit={handleSubmit} className="mt-6 space-y-6">
                  ...
                  <div>
                    ...
                    <input
                      type="datetime-local"
                      value={new Date(formData.estimatedDelivery || "")
                        .toISOString()
                        .slice(0, 16)}
                      onChange={(e) =>
                        setFormData({
    ...
    <>
      <tr className="hover:bg-gray-50">
        ...
        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
          <span
            className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${getStatusColor(order.orderStatus)}`}
          >
            {order.orderStatus}
          </span>
        </td>
        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
          <span
            className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${getStatusColor(order.paymentStatus)}`}
          >
            {order.paymentStatus}
          </span>
        </td>
          ...
          >
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div
                  className={`bg-white rounded-lg shadow-sm p-6 border h-full flex flex-col ${
...
export default function Orders() {
  ...
  const filteredOrders = mockOrderResponse.data.filter(
    (order) =>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-300">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {filterOrdersByType(
                      filteredOrders,
                      tab.name === "All Orders" ? null : tab.name,
                    ).map((order) => (
                      <OrderRow
                        key={order._id}
                        order={order}
