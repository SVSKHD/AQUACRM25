...
const analyticsData = {
  ...
  recentOrders: [
    { id: '1', number: 'ORD-2025-001', customer: 'John Doe', status: 'processing', total: 16500 },
    { id: '2', number: 'ORD-2025-002', customer: 'Jane Smith', status: 'shipped', total: 25000 },
    { id: '3', number: 'ORD-2025-003', customer: 'Mike Johnson', status: 'pending', total: 4000 }
  ]
};
  ...
  return (
    <div className="mb-8 space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        {/* Recent Orders */}
        <div className="bg-white rounded-lg shadow-lg shadow-cyan-100/50 border border-cyan-100 p-6">
          <h3 className="text-lg font-semibold text-gray-900 font-mono mb-4">Recent Orders</h3>
          <div className="space-y-4">
              ...
              <div key={order.id} className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-mono text-gray-500">{order.customer}</p>
                  </div>
                </div>
                <div className="text-right flex items-center space-x-4">
                  <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${getOrderStatusColor(order.status)}`}>
                    {order.status.charAt(0).toUpperCase() + order.status.slice(1)}
                  </span>
                  <span className="font-mono text-gray-900">₹{order.total.toLocaleString()}</span>
                </div>
              </div>
            ))}
          </div>
...
export default function Dashboard() {
  ...
  const { unreadCount, items: notifications } = useAppSelector(state => state.notifications);
  ...
  const tabs: TabItem[] = [
    ...
    { name: 'Notifications', icon: Bell, component: Notifications, notificationCount: unreadCount },
